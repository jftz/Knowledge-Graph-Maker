...
RELATIONSHIP_EXTRACTION_SERVICE_URL = 'http://localhost:4000'
...
def extract_data_from_file(file_path) -> str:    #Doesn't like how the file_path is written/passed

    
    with open(file_path, encoding=ENCODING ) as fd: #Hard coding one of the files may have to edit function
        data = fd.read()
        #file_path
       # , encoding=ENCODING
...
def getDoc():
    ...
    print("select one of the following documents ",'\n' , fileList)
    choice = input()
    while present == False:
        if choice in fileList:
            present = True
        else:
            print("sorry invalid please select one of the following documents ",'\n' , fileList)
            choice = input()
...
#         for line in f:
    ...
    for data_file in data_files:
        data = extract_data_from_file(loc)
...
def main():
   ...
   # isFile = os.path.isfile(data_files) 
